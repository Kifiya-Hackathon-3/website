...
This document complements [features.md](./features.md) with **C4 model views** (context, container, component) and **sequence diagrams** for the main HTTP-driven flows. Diagrams use [Mermaid](https://mermaid.js.org/); render in GitHub, GitLab, VS Code (Mermaid preview), or Confluence (Mermaid macro).
...
| Doc | Purpose |
|-----|---------|
| [features.md](./features.md) | Actor capabilities vs HTTP surface |
| [endpoints-from-architecture.md](./endpoints-from-architecture.md) | All HTTP routes with auth, request, and response summary (aligned with OpenAPI) |
| `../api/docs/registration-api.md` | Registration curl examples (if using `api` repo) |
| [api/openapi.yaml](./api/openapi.yaml) | Authoritative paths and schemas |
...
This section documents **optional** integrations described in [features.md](./features.md) under *Platform admin → Operator assistance*. It does **not** change the Ahadu API trust model: **verify/suspend** remain authoritative on the API with **admin** JWT and audit; external agents are **not** first-class components inside the API process.
...
| Topic | Detail in [features.md](./features.md) |
|-------|----------------------------------------|
